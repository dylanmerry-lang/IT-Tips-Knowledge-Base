// File#: _1_swipe-content (CodyHouse)
(function() {
  var flexSupported = CSS.supports('align-items', 'stretch');
  var transitionSupported = CSS.supports('transition', 'transform');
  var cssPropertiesSupported = ('CSS' in window && CSS.supports('color', 'var(--color-var)'));
...
(function() {
  ...
  window.initTipsCarousel = function(force) {
    var carousels = document.getElementsByClassName('js-carousel');
    if (carousels.length > 0) {
      for (var i = 0; i < carousels.length; i++) {
        (function(i){
          if (force && carousels[i]._carouselInstance) {
            var clones = carousels[i].querySelectorAll('.js-clone');
            for (var c = 0; c < clones.length; c++) {
              clones[c].parentNode.removeChild(clones[c]);
            }
            var list = carousels[i].querySelector('.carousel__list');
            if (list) {
              list.classList.remove('carousel__list--animating');
              list.style.transform = '';
            }
            carousels[i].classList.remove('carousel--loaded');
            carousels[i]._carouselInstance = null;
          }

          if (!carousels[i]._carouselInstance || force) {
